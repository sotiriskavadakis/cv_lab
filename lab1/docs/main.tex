...
Για την εκτίμηση της Laplacian υλοποιήθηκαν δύο εναλλακτικές προσεγγίσεις.
Η εξομάλυνση με Gaussian είναι θεμελιώδης και για τις δύο: η Laplacian είναι
τελεστής δεύτερης παραγώγου και άρα εξαιρετικά ευαίσθητη στον θόρυβο, καθώς
η παραγώγιση ενισχύει τις υψηλές συχνότητες. Και στις δύο περιπτώσεις
χρησιμοποιείται το \emph{σταυροειδές} στοιχείο δόμησης (5-γειτονιά):
\[
B = \begin{pmatrix} 0 & 1 & 0 \\ 1 & 1 & 1 \\ 0 & 1 & 0 \end{pmatrix}
\]
το οποίο αντιστοιχεί στη $d_1$ (cityblock / Manhattan) μετρική, σε αντίθεση
με ένα πλήρες τετράγωνο $3\!\times\!3$ που θα αντιστοιχούσε στη $d_\infty$
(Chebyshev).
...
\begin{tikzpicture}[
    ...
    \node                (in)   {$I$};
    \node[op, right=of in]  (conv) {Συνέλιξη LoG\\$*\;h$};
    \node[right=of conv]    (out)  {$L_1$};
    \draw[arr] (in)   -- node[above, font=\small]{είσοδος} (conv);
    \draw[arr] (conv) -- node[above, font=\small]{αποτέλεσμα} (out);
\end{tikzpicture}
\caption{Ροή υπολογισμού της γραμμικής Laplacian $L_1$: μία συνέλιξη
         της εικόνας εισόδου $I$ με τον πυρήνα LoG $h$.}
\label{fig:l1_flow}
\end{figure}
Εκμεταλλεύεται την ιδιότητα ότι ο τελεστής Laplace επιτρέπει εναλλαγή με τη
συνέλιξη:
\begin{equation}
    L_1 = \nabla^2(G_\sigma * I) = (\nabla^2 G_\sigma) * I = h * I
\end{equation}
Αρκεί δηλαδή μία μόνο συνέλιξη με τον πυρήνα LoG $h$, αντί για ξεχωριστή
εξομάλυνση και εφαρμογή Laplacian. Ο $L_1$ είναι \emph{γραμμικός,
μετατοπιστικά αναλλοίωτος} (LSI) τελεστής: η απόκριση σε κάθε pixel είναι
σταθμισμένος μέσος όρος γειτονικών τιμών, με βάρη που ορίζει ο πυρήνας.
Η γεωμετρία του είναι \emph{ισοτροπική}, αφού το LoG εξαρτάται μόνο από
$r^2=x^2+y^2$ --- δεν ευνοεί καμία κατεύθυνση ακμής. Αυτό τον καθιστά
ιδανικό για φυσικές εικόνες όπου οι ακμές εμφανίζονται σε αυθαίρετες
κατευθύνσεις. Επιπλέον, η Gaussian εξομάλυνση ελαχιστοποιεί το Mean Squared
Error υπό Gaussian θόρυβο --- ακριβώς τον τύπο θορύβου που χρησιμοποιούμε
στα πειράματα --- γεγονός που καθιστά την $L_1$ θεωρητικά βέλτιστη για αυτές
τις συνθήκες. Το κύριο μειονέκτημά της είναι η \emph{διάχυση σφάλματος}: ο
θόρυβος δεν εξαλείφεται αλλά απλώνεται χωρικά μέσω της συνέλιξης, οπότε σε
υψηλό θόρυβο (PSNR $=10\,\text{dB}$) μπορεί να εμφανιστούν ψευδείς ακμές
ακόμη και μετά την εξομάλυνση.
...
\begin{tikzpicture}[
    ...
    \node                   (in)  {$I$};
    \node[op, right=of in]  (gs)  {Εξομάλυνση\\$G_\sigma$};
...
\caption{Ροή υπολογισμού της μη-γραμμικής Laplacian $L_2$: η εξομαλυμένη
         ...
         και τα αποτελέσματα συνδυάζονται ως
         $(\oplus)+(\ominus)-2I_\sigma$.}
\label{fig:l2_flow}
\end{figure}

Βασίζεται σε τελεστές Min-Max εφαρμοζόμενους στην εξομαλυμένη εικόνα
$I_\sigma$ \cite{maragos2005}:
\begin{equation}
    L_2 = (I_\sigma \oplus B) + (I_\sigma \ominus B) - 2I_\sigma
\end{equation}
Η dilation $(I_\sigma \oplus B)$ επιλέγει τη \emph{μέγιστη} τιμή στη
5-γειτονιά κάθε pixel, ενώ η erosion $(I_\sigma \ominus B)$ την
\emph{ελάχιστη}. Ο $L_2$ μετρά έτσι την απόκλιση του κεντρικού pixel
από τον μέσο όρο των τοπικών ακρoτάτων --- ανάλογη φυσική ερμηνεία με
τον κλασικό τύπο $f(x{+}h)+f(x{-}h)-2f(x)\approx h^2f''(x)$.
Η εξομάλυνση πρέπει να εφαρμοστεί \emph{ρητά} πριν τους μορφολογικούς
τελεστές, αφού η μη-γραμμικότητα των max/min δεν επιτρέπει ενοποίησή τους
σε ενιαίο πυρήνα \cite{maragos2005}. Το κύριο \emph{πλεονέκτημα} των
μορφολογικών τελεστών είναι η ανθεκτικότητα σε ακραίες τιμές: τα min/max
λειτουργούν ως rank filters που καταστέλλουν αποτελεσματικά θόρυβο τύπου
salt-and-pepper, σε αντίθεση με τη γραμμική συνέλιξη που απλώνει τέτοιον
θόρυβο. Ωστόσο, η μέθοδος \emph{υστερεί} στα πειράματά μας επειδή ο
θόρυβος είναι Gaussian: οι τελεστές max/min λαμβάνουν υπόψη μόνο τις ακραίες
τιμές στη γειτονιά (αντί για σταθμισμένο μέσο όρο), παράγοντας παχύτερες
και πιο αποσπασματικές ακμές. Επιπλέον, λόγω του σταυροειδούς $B$, η
απόκριση δεν είναι ισοτροπική: ευνοεί τις αξονικές (οριζόντιες/κάθετες)
ακμές, ενώ οι διαγώνιες ακμές ανιχνεύονται λιγότερο αξιόπιστα.
